...
function  UserSwap ({ isOpen, onClose, schedule }){
    ...
    useEffect(() => {
        ...
        if (schedule.id != null) {
            getUserListToSwapAssignSchedule(schedule.id);
        }
    }, [schedule]);
    ...
    const getUserListToSwapAssignSchedule = async (id) => {
        const request = {
            scheduleId: id,
        };
        const response = await fetch(
            `http://localhost:5008/api/Schedule/GetUserListToSwapAssignSchedule`,
            {
                method: "POST",
                headers: {
    ...
    const handleUserAssign = async () => {
        ...
        const request = {
            ...
            swapUserId: selectedUser,
            rejectUserId:schedule.rejectedUserId
        };

        if (selectedUser !== "" && schedule.id != null) {
            const response = await fetch(
                `http://localhost:5008/api/Schedule/SwapScheduleToUser`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
              ...
              <div className="flex justify-between items-center mb-4">
                <div className="flex items-center">
                  <h2 className="text-lg font-semibold">
                    Swap Users to {schedule.name.slice(0, 5)}
                  </h2>
                </div>
                <button
                  onClick={onClose}
                ...
                {filteredUsers.map((user, index) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">Approve Points : {user.approvalPointCount}</p>
                    </div>
                  </div>
                ))}
              </div>
