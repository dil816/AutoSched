...
function UserAssign({ isOpen, onClose, schedule }) {
  ...
  useEffect(() => {
    ...
    if (schedule.id != null) {
      getUserListToAssignSchedule(schedule.id);
    }
  }, [schedule]);
  ...
  const getUserListToAssignSchedule = async (id) => {
    const request = {
      scheduleId: id,
    };
    const response = await fetch(
      `http://localhost:5008/api/Schedule/GetUserListToAssignSchedule`,
      {
        method: "POST",
        headers: {
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center">
              <h2 className="text-lg font-semibold">
                Assign Users to {schedule.name.slice(0, 5)}
              </h2>
            </div>
            <button
              onClick={onClose}
